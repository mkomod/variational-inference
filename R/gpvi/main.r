...
# Plot data, and data generating process
local({
...
# Making predictions ---------------------------------------------------------
pdf("./plots/gpvi_fit.pdf", width=9, height=5)
par(mar=c(4,3,0,0))
local({
    d.hmc <- read.csv("./results/fitsummary.csv")
    d.hmc <- d.hmc[22:41, ]
    plot(X, d.hmc$mean, type="l", col="black",
	bty="n", ylim=c(-10, 5), ylab="")
    polygon(c(X, rev(X)), c(d.hmc$X97.5., rev(d.hmc$X2.5.)), 
    col=rgb(0, 0, 0, 0.2), border=NA)
})
local({
    ...
    points(X, Y, pch=17, col="blue")
    points(X, f, pch=15, col="darkgreen")
    legend(8, -6, legend=c("Y", "f(x)", "f(x) VI", "f(x) HMC"), 
	   col=c("blue", "darkgreen", "red", "black"), 
	   lty=c(NA, NA, 1, 1), pch=c(17, 15, NA, NA))
})
dev.off()
...
pdf("./plots/gpvi_vsbc.pdf", width=12.5, height=2.5)
par(mfrow=c(1,5), mar=c(3, 4, 2, 2))
hist(vsbc.mat[ , 1], main="", xlab="")
par(mar=c(3, 3, 2, 2))
hist(vsbc.mat[ , 5], main="", xlab="")
hist(vsbc.mat[ , 10], main="", xlab="")
hist(vsbc.mat[ , 15], main="", xlab="")
hist(vsbc.mat[ , 20], main="", xlab="")
dev.off()
