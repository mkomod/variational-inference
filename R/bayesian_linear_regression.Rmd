...
elbo <- function(n, K, mu, Sigma, a, a2, b, b2, varBeta) {
  ...
  # p1 and p3 are constant in the algorithm
  # for numerical stability we exclude them
  elb <- p2 + p4 + p5
...
#   b2 <- b + .5*(t(u) %*% u) + .5*sum(diag(Sigma %*% v))
#   
#   elbotemp <- elbo(n, K, mu2, Sigma2, a, a2, b, b2, varBeta)
#   print(elbotemp)
# }
```
```{r cavi}
...
for(i in c(1,2,101)) {
  ...
  hist(probs[, i], breaks = ss, axes = F, xlim = c(0,1), xlab = "", ylab = "", main = "", ylim = c(0, 8), probability = T, xpd=T)
  ...
  if (i == 1) {
    axis(2, at = seq(from = 0, to = 8, by = 2), lwd = .5, las = 2)
  ...
  if (i == 101){
    mtext(3, text = paste(" p=", round(ksTest$p.value,digits = 101)), cex = .7, line = -1.25)
...
logjointpdf <- function(X, beta, var) {
  ...
  p3 <- sum(dinvgamma(var, 1, 1, log = TRUE))
  return (p1 + p2 + p3)
}
...
jd = psis(log_ratios = logisratio, r_eff = NA, cores = 4)
